...
//    }
    ...
    public String messageList(Model model, HttpServletRequest req) {
        ...
        List<Message> messageList = messageService.getMessageList(memberId);
        model.addAttribute("messages", messageList);
        return "message/list";
    }
    ...
    // 쪽지 등록
    @PostMapping("/regist")
    public String registMessage(@RequestParam String receiverId, @RequestParam String content, HttpServletRequest req, Model model) {
        String senderId = getMemberIdInJwt(req);
        if(senderId == null) {
            return "redirect:/sign/signIn";
        }
